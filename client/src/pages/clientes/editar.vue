      ...
      <template #content>
        <form @submit.prevent="onSubmit" class="flex flex-col gap-4">
          <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div class="flex flex-col gap-1">
              <label class="font-medium text-sm">CPF/CNPJ</label>
              <InputText v-model="form.cpf_cnpj" placeholder="CPF ou CNPJ" />
            </div>
            <div class="flex flex-col gap-1">
              <label class="font-medium text-sm">Tipo de Pessoa</label>
              <Select v-model="form.tipo_pessoa" :options="tiposPessoa" optionLabel="label" optionValue="value" placeholder="Selecione" class="w-full" />
            </div>
            <div class="flex flex-col gap-1">
            ...
            <div class="flex flex-col gap-1">
              <label class="font-medium text-sm">Celular</label>
            ...
            <div v-for="(endereco, idx) in form.enderecos" :key="idx" class="border rounded p-3 mb-2">
              <div class="grid grid-cols-1 md:grid-cols-3 gap-3">
                <div class="flex flex-col gap-1">
                  ...
                  <InputText v-model="endereco.cep" placeholder="00000-000" />
                </div>
                <div class="flex flex-col gap-1 md:col-span-2">
                  <label class="font-medium text-sm">Logradouro</label>
                  <InputText v-model="endereco.logradouro" placeholder="Rua, Avenida..." />
                </div>
                <div class="flex flex-col gap-1">
                  <label class="font-medium text-sm">Numero</label>
                  <InputText v-model="endereco.numero" placeholder="Numero" />
                </div>
                <div class="flex flex-col gap-1">
                  <label class="font-medium text-sm">Complemento</label>
                  <InputText v-model="endereco.complemento" placeholder="Apto, Sala..." />
                </div>
                <div class="flex flex-col gap-1">
                  <label class="font-medium text-sm">Bairro</label>
                  <InputText v-model="endereco.bairro" placeholder="Bairro" />
                </div>
                <div class="flex flex-col gap-1">
                  <label class="font-medium text-sm">Cidade</label>
                  <InputText v-model="endereco.cidade" placeholder="Cidade" />
                </div>
                <div class="flex flex-col gap-1">
                  <label class="font-medium text-sm">UF</label>
                  <InputText v-model="endereco.uf" placeholder="UF" maxlength="2" />
                </div>
                <div class="flex items-end">
                  <Button type="button" icon="pi pi-trash" severity="danger" text @click="removeEndereco(idx)" />
                </div>
              </div>
            </div>
          </div>
          ...
          <div>
            ...
            <div v-for="(contato, idx) in form.contatos" :key="idx" class="border rounded p-3 mb-2">
              <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
                <div class="flex flex-col gap-1">
                  <label class="font-medium text-sm">Nome</label>
                  <InputText v-model="contato.nome" placeholder="Nome do contato" />
                </div>
                <div class="flex flex-col gap-1">
                  <label class="font-medium text-sm">Cargo</label>
                  <InputText v-model="contato.cargo" placeholder="Cargo" />
                </div>
                <div class="flex flex-col gap-1">
                  <label class="font-medium text-sm">Telefone</label>
                  <InputText v-model="contato.telefone" placeholder="(00) 0000-0000" />
                </div>
                <div class="flex flex-col gap-1">
                  <label class="font-medium text-sm">Email</label>
                  <InputText v-model="contato.email" type="email" placeholder="email@exemplo.com" />
                </div>
                <div class="flex items-end">
                  <Button type="button" icon="pi pi-trash" severity="danger" text @click="removeContato(idx)" />
                </div>
              </div>
            </div>
          </div>
...
const form = reactive({
  nome: '',
  cpf_cnpj: '',
  tipo_pessoa: null as string | null,
  email: '',
  telefone: '',
  celular: '',
  observacoes: '',
...
function addEndereco() {
  form.enderecos.push({ cep: '', logradouro: '', numero: '', complemento: '', bairro: '', cidade: '', uf: '' })
}
...
const { loading, errors, submit } = useForm((data: any) => clientesService.editar(route.params.id as string, data))
...
onMounted(async () => {
  ...
  try {
    ...
    const d = res.data
    form.nome = d.nome || ''
    form.cpf_cnpj = d.cpf_cnpj || ''
    form.tipo_pessoa = d.tipo_pessoa || null
    form.email = d.email || ''
    form.telefone = d.telefone || ''
    form.celular = d.celular || ''
    form.observacoes = d.observacoes || ''
    form.enderecos = d.enderecos || []
    form.contatos = d.contatos || []
  } finally {

      ...
      <template #content>
        <form @submit.prevent="onSubmit" class="flex flex-col gap-4">
          <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div class="flex flex-col gap-1"><label class="font-medium text-sm">Nome Fantasia</label><InputText v-model="form.nome_fantasia" placeholder="Nome fantasia" /></div>
            <div class="flex flex-col gap-1"><label class="font-medium text-sm">CNPJ</label><InputText v-model="form.cnpj" placeholder="00.000.000/0000-00" /></div>
            <div class="flex flex-col gap-1"><label class="font-medium text-sm">Email</label><InputText v-model="form.email" type="email" placeholder="email@exemplo.com" /></div>
            <div class="flex flex-col gap-1"><label class="font-medium text-sm">Telefone</label><InputText v-model="form.telefone" placeholder="(00) 0000-0000" /></div>
          </div>
          <div class="flex flex-col gap-1"><label class="font-medium text-sm">Observacoes</label><Textarea v-model="form.observacoes" rows="3" placeholder="Observacoes" /></div>
          <!-- Enderecos -->
          <div>
...
const { loading, errors, submit } = useForm((data: any) => fornecedoresService.editar(route.params.id as string, data))
onMounted(async () => {
  fetching.value = true
  try {
    const { data: res } = await fornecedoresService.buscar(route.params.id as string)
    const d = res.data
    form.razao_social = d.razao_social || ''
    form.nome_fantasia = d.nome_fantasia || ''
    form.cnpj = d.cnpj || ''
    form.email = d.email || ''
